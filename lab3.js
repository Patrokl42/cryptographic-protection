...
const encryption = (msg, key) => des(msg, key, keySchedule(key));
const decryption = (msg, key) => des(msg, key, keySchedule(key).reverse());

const message = "0123456789ABCDEF";  // hex
const key = "133457799BBCDFF1";  // hex

console.log(`Початкове повідомлення: "${message}"`);
console.log(`Ключ: "${key}"`);

const msgToBin = bin("0123456789ABCDEF");
const keyToBin = bin("133457799BBCDFF1");

const encryptedMessage = encryption(msgToBin, keyToBin);

console.log(`Закодоване повідомлення: "${encryptedMessage}"`);

const decryptedText = decryption(bin(encryptedMessage), keyToBin);

console.log(`Розкодоване повідомлення: "${decryptedText}"`);
